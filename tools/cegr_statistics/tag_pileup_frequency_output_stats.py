...
parser.add_argument('--input_heatmap', dest='input_heatmaps', action='append', nargs=5, help='HTML input datasets and attributes')
parser.add_argument('--input_tabular', dest='input_tabulars', action='append', nargs=5, help='Text input datasets and attributes')
...
for input_heatmap in args.input_heatmaps:
    file_path, hid, input_id, input_datatype, dbkey = input_heatmap
    if payload_dbkey is None:
        payload_dbkey = dbkey
    statistics.append({})
    datasets.append(stats_util.get_datasets(args.config_file, input_id, input_datatype))
for input_tabular in args.input_tabulars:
    file_path, hid, input_id, input_datatype, dbkey = input_tabular
    statistics.append({})
    datasets.append(stats_util.get_datasets(args.config_file, input_id, input_datatype))
